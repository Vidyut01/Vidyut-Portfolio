...
const Section = ({id, title, children, className, reverse = false}: {
  ...
  return (
    <section id={id} className={`flex flex-col items-center lg:flex-row lg:items-start justify-between overflow-hidden px-12 lg:px-32 mb-10 w-full ${className}`}>
      ...
      >
        {!reverse && <div className={`text-5xl w-[30%] h-full border-r grid justify-center pt-10 border-[#E6E6FA] ${headingFont.className}`}>
          {title}
        </div>}
        <div className="flex flex-col items-center w-[70%]">
          {children}
        </div>
        {reverse && <div className={`text-5xl w-[30%] h-full border-l grid justify-center pt-10 border-[#E6E6FA] ${headingFont.className}`}>
          {title}
        </div>}
      </motion.article>
    </section>
